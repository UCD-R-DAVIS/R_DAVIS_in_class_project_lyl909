...
surveys$hindfoot_cat <- ifelse(surveys$hindfoot_length < mean
                               (surveys$hindfoot_length, na.rm = TRUE),yes ="small", no = "big") ##we need to remove the na, if na.rm=FALSE that will be something wrong
...
surveys %>%
  mutate(hindfoot_cat = case_when(
    hindfoot_length > 29.29 ~ "big", # hindfoot lenght over mean (29.29) I want to be reclassified as "big"
    TRUE ~ "small" #"else" part: except larger than 29.29 is big, and other will be small
  )) %>%
...
d <- surveys%>%
  mutate(hindfoot_cat = case_when(
    hindfoot_length >31.5 ~"big", ##use , to connect all the staff
